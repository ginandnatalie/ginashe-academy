...
export default function StreamRequirements({ onOpenModal, editMode }: StreamRequirementsProps) {
  ...
  return (
    <div className="min-h-screen bg-bg pt-12 pb-20">
      <SEO 
        title={`${stream.title} | Requirements`}
        description={`Admission criteria, assessment parameters, and computer specifications for the ${stream.title} (${stream.abbr}).`}
      />
      <div className="max-w-4xl mx-auto px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-syne font-black text-text-custom mb-6">Who Can Apply?</h1>
          
          <div className="bg-glass-bg border border-border-custom p-8 rounded-3xl mb-8">
            <h2 className="font-syne font-bold text-2xl text-text-custom mb-4">Aptitude over Academics</h2>
          ...
          <div className="space-y-6">
            <h2 className="font-syne font-bold text-2xl text-text-custom">Course-Specific Prerequisites</h2>
            {stream.curriculum.map((course, idx) => (
              <div key={idx} className="bg-glass-bg border border-border-custom p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h3 className="font-syne font-bold text-text-custom text-lg mb-1">{course.title}</h3>
