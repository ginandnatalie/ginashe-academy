                ...
                <Link to={stream.path} className={`block rounded-2xl border ${stream.border} bg-glass-bg hover:bg-glass-bg transition-all group relative overflow-hidden no-underline`}>
                  <div className="flex flex-col lg:flex-row">
                    ...
                    <div className="p-8 lg:w-[320px] shrink-0 flex flex-col items-start">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className={`text-[9px] font-jetbrains uppercase tracking-widest px-2 py-0.5 rounded border ${(stream.status === 'Live' || stream.status === 'Enrolling') ? 'bg-brand/10 text-brand border-brand/20' : 'bg-glass-bg text-text-muted border-border2'}`}>{stream.status}</span>
