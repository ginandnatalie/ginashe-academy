    ...
    <div className="min-h-screen bg-bg pt-12 pb-20">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-syne font-black text-white mb-6">Curriculum & Pathways</h1>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
            {stream.curriculum.map((course, idx) => {
              const meta = parseMeta(course.meta);
              return (
                <div key={idx} className="bg-white/[0.02] border border-white/5 rounded-3xl p-6 flex flex-col hover:border-brand/30 transition-all group">
                  ...
                  <h3 className="font-syne font-bold text-xl text-white mb-4 group-hover:text-brand transition-colors">{course.title}</h3>
                  
                  <div className="flex flex-wrap gap-2 mb-6">
                    {meta.duration && (
                      <div className="flex items-center gap-1.5 text-xs text-text-muted bg-white/5 px-2 py-1 rounded">
                  ...
                  <div className="pt-4 border-t border-white/5 mt-auto">
                    ...
                    <div className="text-sm text-white font-medium">{course.outcome}</div>
                  </div>
                </div>
              );
            })}
