      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-10 lg:gap-16 mb-16 items-start">
          <div>
            ...
            <h2 className="font-syne font-extrabold text-4xl mb-6 text-text-custom">Who can apply to<br />the Academy?</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div className="space-y-6">
              {criteria.map((c, i) => (
                <div key={i} className="flex gap-4 p-4 rounded-2xl bg-white/3 border border-border-custom">
                  ...
                  <div>
                    <h3 className="font-bold text-sm text-text-custom">{c.title}</h3>
          ...
          <div className="relative">
            <div className="aspect-square rounded-3xl overflow-hidden border border-border-custom bg-navy relative">
              ...
              <div className="absolute bottom-10 left-10 right-10 p-8 bg-card backdrop-blur-md rounded-2xl border border-border2">
                <div className="text-brand text-2xl font-black mb-1">98%</div>
                <div className="text-[10px] uppercase tracking-widest text-text-muted">Employment Rate Post-Placement</div>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <section id="tuition" className="bg-navy py-24 border-t border-border-custom px-6 sm:px-14 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto text-center relative z-10">
        <h2 className="font-syne font-extrabold text-4xl mb-4 text-text-custom">Tuition & Investment</h2>
              ...
              <div className="grid grid-cols-2 gap-3">
                {Object.keys(levelData).map((level) => (
                  <button
                    ...
                    className={`px-4 py-3 rounded-xl font-syne font-bold text-[11px] transition-all border ${activeLevel === level ? 'bg-brand border-brand text-navy' : 'bg-glass-bg border-border-custom text-text-muted hover:border-brand/30'}`}
                  >
                    {level}
                  </button>
                ))}
            ...
            <div className="text-left h-full flex flex-col justify-end">
              ...
              <div className="relative">
                <select 
                  ...
                  className="w-full bg-surface border border-border-custom rounded-xl p-4 px-6 font-syne font-bold text-[13px] text-text-custom outline-none focus:border-brand/40 transition-all appearance-none cursor-pointer"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan, i) => (
            <div key={i} className={`p-8 rounded-3xl border transition-all flex flex-col ${plan.popular ? 'bg-brand border-brand scale-105 z-20 shadow-2xl' : 'bg-white/3 border-border-custom hover:border-brand/30'}`}>
              <h3 className={`font-syne font-bold text-lg mb-4 ${plan.popular ? 'text-navy' : 'text-text-custom'}`}>{plan.title}</h3>
              ...
              <button 
                ...
                className={`w-full py-3 rounded-xl font-bold text-sm transition-all ${plan.popular ? 'bg-navy text-text-custom hover:bg-navy/80' : 'bg-brand text-navy hover:bg-brand-dim'}`}
              >
